    ...
    for motion_loader_name, motion_loader in motion_loaders.items():
        ...
        with torch.no_grad():
            for idx, batch in enumerate(motion_loader):
                word_embeddings, _, sent_lens, motions, m_lens, _ = batch
...
def evaluate_fid(eval_wrapper, groundtruth_loader, activation_dict, file):
    ...
    with torch.no_grad():
        for idx, batch in enumerate(groundtruth_loader):
            _, _, sent_lens, motions, m_lens, _ = batch
...
if __name__ == '__main__':
    ...
    split = 'train'
    gt_loader = get_dataset_loader(name='openaslgt', batch_size=args.batch_size, num_frames=None, split=split, hml_mode='gt')
