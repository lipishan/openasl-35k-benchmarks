...
def get_dataset_class(name):
    if name == "amass":
        ...
        from data_loaders.humanml.data.sign_datasets import OpenASL
        return OpenASL
    elif name == "openaslgt":
        from data_loaders.humanml.data.signgt_datasets import OpenASLGT
        return OpenASLGT
    else:
...
def get_collate_fn(name, hml_mode='train', pred_len=0, batch_size=1):
    if hml_mode == 'gt':
        ...
        from data_loaders.humanml.motion_loaders.model_motion_loaders import collate_signs_fn as t2m_eval_collate
        # collate_signs_fn as t2m_eval_collate
        return t2m_eval_collate
    if name in ["humanml", "kit", "openasl"]:
        if pred_len > 0:
...
def get_dataset(name, num_frames, split='train', hml_mode='train', abs_path='.', fixed_len=0, 
    ...
    if name in ["humanml", "kit", "openasl", "openaslgt"]:
