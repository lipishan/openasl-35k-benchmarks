...
class Text2MotionDatasetV2(data.Dataset):
    def __init__(self, opt, mean, std, split_file, split, w_vectorizer):
        self.opt = opt
        ...
        if not load_direct:
            for name in tqdm(id_list):
                try:
                    ...
                    motion = geometry.matrix_to_rotation_6d(geometry.axis_angle_to_matrix(torch.from_numpy(motion)))
    ...
    def __getitem__(self, item):
        ...
        #     motion_pose = np.concatenate([motion_pose,
        #                              np.zeros((self.max_motion_length - m_length, motion_pose.shape[1]))
        #                              ], axis=0)
        ...
        return word_embeddings, None, caption, sent_len, motion, length, '_'.join(tokens)


# A wrapper class for t2m original dataset for MDM purposes
class OpenASL(data.Dataset):
    def __init__(self, mode, datapath='./dataset/humanml_opt.txt', split="train", **kwargs):
        ...
        #     # used by our models
        self.mean = np.load(pjoin(opt.data_root, 'meta', 'mean.npy'))
        self.std = np.load(pjoin(opt.data_root, 'meta', 'std.npy'))
        ...
        train_split_file = data[data['split'] == 'train']
        val_split_file = data[data['split'] == 'valid']
       
        self.w_vectorizer = BertWordVectorizer()
        self.t2m_dataset = Text2MotionDatasetV2(self.opt, self.mean, self.std, train_split_file, split, self.w_vectorizer)
