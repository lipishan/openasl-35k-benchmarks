...
class CompMDMGeneratedDataset(Dataset):
    ...
    def __init__(self, args, model, diffusion, dataloader, mm_num_samples, mm_num_repeats, max_motion_length, num_samples_limit, scale=1.):
        ...
        with torch.no_grad():
            for i, (motion, model_kwargs) in tqdm(enumerate(dataloader)):
                ...
                tokens = [["[CLS]"] + t.split('_') + ["[SEP]"] for t in model_kwargs['y']['tokens']]
                ...
                for t in range(repeat_times):
                    ...
                    if t == 0:
                        ...
                        sub_dicts = [{
                            'motion': sample[bs_i].squeeze().permute(2, 0, 1).cpu().numpy(),
                            ...
                            'cap_len': tokens[bs_i].index("[SEP]") + 1,
                            } for bs_i in range(dataloader.batch_size)]
                        generated_motion += sub_dicts

                    if is_mm:
                        for bs_i in range(dataloader.batch_size):
                            mm_motion = sample[bs_i].squeeze().permute(2, 0, 1).cpu().numpy()
        ...
        # import pdb; pdb.set_trace()
        with open("model000050000.generated_samples.train.pkl", "wb") as f:
            pickle.dump({'generated_motion': self.generated_motion,
                         'mm_generated_motion': self.mm_generated_motion}, f)
    ...
    def __getitem__(self, item):
        ...
        return word_embeddings, None, caption, sent_len, motion, m_length, '_'.join(tokens)